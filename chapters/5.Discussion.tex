...
%Deepfake technology has advanced within just two years to the point where reenactment can be done in real-time with training requiring only a few images or seconds of audio from the victim, while higher quality deepfakes still require more audio/video data~\citep{mirsky_Threat_Offensive_AI_Organizations_2023}.
...
%As AI is developed further and the more its availability increases, the risk of malicious or criminal use increases as well, and these risks, if not properly addressed, may lead to the excessively strict regulation of AI technologies~\citep{king_AI_Crime_Interdisciplinary_Analysis_2019}.
